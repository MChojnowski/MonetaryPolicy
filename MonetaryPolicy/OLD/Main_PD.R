##### HEAD #####
rm(list=ls(all=TRUE))
setwd("C:/Users/piotr.dybka/Desktop/Monetary_policy")
...
###### wariant 2 z oczekiwaniami adaptacyjnymi
qwe1<-cumsum(qwe)/sd(cumsum(qwe))
plot(qwe1, type="l")

h<-12 #horyzont œredniej
z<-h-1
...
sdp2<-sd(qwe3)
###### liczymy prawdopodobieñstwa
prob2<-pnorm(qwe3,  mean = 0, sd = sdp2)
plot(prob2, type="l")

###### Wariant 3 - bez jakiœ przesadnych przekszta³ceñ sentymentóW
m1<-mean(qwe1)
sdp3<-sd(qwe1)
prob3<-pnorm(qwe1,  mean = m1, sd = sdp3)
plot(prob3, type="l")
