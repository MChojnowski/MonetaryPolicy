...
temp.noGDP<-solve(A)%*%model_svar_1$B
...
plot.ts(irf(model_var_1,impulse="WIBOR",response="CPI",n.ahead=24)$irf[[1]])
lines(IRF,col="blue")
...
model_svar_1_noGDP<-model_svar_1
model_svar_1_noGDP$A[2,1]<-0
model_svar_1_noGDP$var$varresult$CPI$coef[1]<-0

Psi_withGDP<-array(NA,dim=c(4,4,10))
Psi_withGDP[,,1]<-solve(model_svar_1$A)%*%model_svar_1$B
Psi_noGDP<-array(NA,dim=c(4,4,10))
Psi_noGDP[,,1]<-solve(model_svar_1_noGDP$A)%*%model_svar_1_noGDP$B

for(i in 2:10){
	Psi_withGDP[,,i]<-Psi_withGDP[,,i-1]%*%Psi_withGDP[,,i-1]
	Psi_noGDP[,,i]<-Psi_noGDP[,,i-1]%*%(solve(model_svar_1_noGDP$A)%*%model_svar_1_noGDP$B)
}

plot(Psi(model_svar_1$var)[2,3,],type="l")
lines(Psi_withGDP[2,3,],type="p",col="red")
lines(Psi_noGDP[2,3,],type="p",col="blue")
