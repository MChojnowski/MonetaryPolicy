...
wynagrodzenie<-log(dane_m[,"wag"]/dane_m[,"mon_pol"])
...
kredyty<-log(dane_m[,"loa"]/dane_m[,"mon_pol"])
...
if(FALSE){
	...
	mon_pol<-window(mon_pol,start=start.dane)
	wibor<-window(wibor,start=start.dane)
	reer<-window(reer,start=start.dane)
}

#mon_pol.d<-diff(mon_pol)
ev1<-ts(rep(0,length(mon_pol)),start=start(mon_pol),freq=12)
ev1.1<-ts(1,start=c(1997,1),freq=12)
ev1<-ts(apply(cbind(ev1,ev1.1),1,sum,na.rm=TRUE),start=start(mon_pol),freq=12)

model.mon_pol<-arima(mon_pol,order=c(3,0,2),xreg=ev1,method="CSS")
mon_pol<-mon_pol-ev1*model.mon_pol$coef["ev1"]
...
colnames(dane_monet)<-c("GDP","mon_pol","WIBOR","REER")
